...
```{r plot2, echo=FALSE, fig.width=6, fig.height=6, dev=c("png", "pdf")}
library(survival)
library(survminer)
library(ggbreak)
preds<-fread(here("data","All_MICA_withscores.csv"))
surv_cv<-preds 
surv_cv<-surv_cv %>% mutate(censor=if_else(`Death status`==1,0,1)) #This is weird but the metadata clearly states that 1=alive, 2 = dead
...
d$score<-(d$score-mean(d$score))/sd(d$score)
d$cci<-(d$`CCI score`-mean(d$`CCI score`))/sd(d$`CCI score`)
d$age<-(d$age-mean(d$age))/sd(d$age)
d$YKL40<-(d$YKL40-mean(d$YKL40))/sd(d$YKL40)
d$IL6<-(d$IL6-mean(d$IL6))/sd(d$IL6)
d$CRP<-(d$CRP-mean(d$CRP,na.rm = TRUE))/sd(d$CRP,na.rm = TRUE)
d$sex<-(d$sex-mean(d$sex))/sd(d$sex)



full_cph<-coxph(Surv(surv, censor) ~ YKL40 + CRP + IL6  + age + cci + score + sex, data =  d)


cp<-tibble(var=c("YKL40","CRP","IL6","Age","CCI","Fragmentation\nComorbidity\nIndex","Sex"),model="Multivariate",low=data.table(summary(full_cph)$conf.int)[,3],high=data.table(summary(full_cph)$conf.int)[,4],coef=data.table(summary(full_cph)$conf.int)[,1],p=data.table(summary(full_cph)$coefficients)[,5])
colnames(cp)<-c("var","model","low","high","coef","pval")

cp<-data.table(cp)

cp<-cp %>% mutate(sig=if_else(pval<.05,"p<0.05","NS"))
cp<-cp %>% mutate(sig=if_else(pval<.005,"p<0.005",sig))
cp$sig<-factor(cp$sig,levels=c("NS","p<0.05","p<0.005"))
cp$var<-factor(cp$var,levels=rev(c("YKL40","CRP","IL6","Age","CCI","Fragmentation\nComorbidity\nIndex","Sex")))
...
```{r plot, echo=FALSE, fig.width=12, fig.height=5, dev=c("png", "pdf")}

#p1<- ggplot(cp %>% filter(model=="Multivariate"),aes(y=var,x=coef,color=sig))+geom_point()+theme_classic()+geom_vline(xintercept=1,linetype="dashed",color="black")+geom_errorbar(aes(y=var,xmin=low,xmax=high,width=.5))+ylab("")+xlab("")+theme(legend.position="none",axis.text.x.top = element_blank(),
#        axis.ticks.x.top = element_blank(),
#        axis.line.x.top = element_blank(),
#        axis.line.y=element_blank(), axis.text.y=element_blank(), #axis.ticks.y=element_blank(),axis.text.x=element_text(color="black",face="bold"))+scale_color_manual(values=c("gray50","hotpink","darkgreen"))+labs(color="#")+scale_x_break(c(2, 52),ticklabels=c(55))+scale_x_break(c(56,2700),ticklabels=c(2770))+scale_x_log10()




cox_tb<-d %>% get_hr_tbl(full_cph, status = "censor")
var_labels <- c("","YKL40", 
                "","CRP",
                "","IL6",
                "", "Age",
                ...
                "","Fragmentation\nComorbidity Index",
                "","Sex")
test<-plot_hr(cox_tb, "test_fig_intg", var_labels=var_labels)
